...
import { uploadUserPostRentalImages } from "@/lib/api";
...
interface PostRentalImageUploadProps {
    booking: Booking;
    open: boolean;
...
export function PostRentalImageUpload({ 
    booking, 
    open, 
    onOpenChange, 
    onSuccess 
}: PostRentalImageUploadProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedImages.length === 0) {
            toast.error("Please select at least one image showing the vehicle condition");
            return;
        }
        ...
        try {
            ...
            selectedImages.forEach((image) => {
                formData.append("userPostRentalImages", image);
            });
            ...
            const response = await uploadUserPostRentalImages(booking._id, formData);

            if (response.success) {
                toast.success("Vehicle condition images uploaded successfully!");
                onSuccess();
                onOpenChange(false);
                // Reset form
                setSelectedImages([]);
    ...
    const alreadyUploaded = (booking.userPostRentalImages && booking.userPostRentalImages.length > 0) || 
                ...
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            ...
                            {(booking.userPostRentalImagesUploadedAt || booking.postRentalImagesUploadedAt) && 
                                new Date(booking.userPostRentalImagesUploadedAt || booking.postRentalImagesUploadedAt!).toLocaleDateString()
                            }.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
            ...
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            You can upload vehicle condition images after your rental period ends on{" "}
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Please upload photos showing the current condition of <strong>{booking.vehicleId.name}</strong> after your rental period.
                        This helps ensure transparency and proper security deposit processing.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="space-y-2">
                        <Label htmlFor="images">
                            Vehicle Condition Images <span className="text-destructive">*</span>
                        </Label>
